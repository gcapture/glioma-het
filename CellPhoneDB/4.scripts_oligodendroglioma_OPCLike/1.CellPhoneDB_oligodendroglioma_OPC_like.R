...
#1st. primary oligodendroglioma microglia cells extraction
...
#2nd. OPC like oligodendroglioma primary cells extraction
...
#log normalize OPC like oligodendroglioma cells
...
cpdb <- merge(seurat, y = Clusters, add.cell.ids = c("oligo", "Microglia"), merge.data = TRUE)
...
write.table(counts,
            "counts_oligo_OPC_like_microgliaAll.txt",
            sep="\t",
            quote=FALSE,
            row.names = TRUE)
...
write.table(metadata,
            "metadata_oligo_OPC_like_microgliaAll.txt",
            sep="\t",
            quote=FALSE,
            row.names = FALSE)
