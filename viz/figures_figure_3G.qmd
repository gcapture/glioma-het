...
macro_ligand <- c("SPP1", "TNF", "DLL1", "SIRPA", "WNT5A", "LGALS9", "PDGFB")
...
interact_df <- interact_df %>% 
               dplyr::filter(.data$source %in% setdiff(unique(interact_df$target), tumor) & .data$ligand.complex %in% macro_ligand) %>% 
               dplyr::filter(.data$target %in% tumor) %>% 
               dplyr::filter(.data$aggregate_rank < 0.05) %>%
               dplyr::mutate(target = ifelse(.data$target %in% tumor, .data$disease, .data$target),
                             aggregate_rank_inv = -log10(aggregate_rank + 1e-10)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "BMD_anti−inflammatory_TAMs", "BMD Anti-inflammatory TAMs", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_Activated", "Mg Activated", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_homeostatic", "Mg Homeostatic", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_IFNg_TAMs", "Mg IFNg TAMs", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_Inflammatory_ICAM+", "Mg Inflammatory ICAM1+", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_inflammatory_TAMs", "Mg Inflammatory TAMs", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_phagocytic", "Mg Phagocytic", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_resident−like_TAMs", "Mg Resident−like TAMs", .data$source)) %>% 
               dplyr::mutate("source" = ifelse(.data$source == "Mg_stressed_TAMs", "Mg Stressed TAMs", .data$source))


liana_OD <- interact_df  %>% 
            dplyr::filter(.data$target %in% c("Astrolike_Oligodendroglioma", "OPClike_Oligodendroglioma")) %>% 
            dplyr::mutate("target" = ifelse(.data$target == "Astrolike_Oligodendroglioma", "Astro-like", "OPC-like")) %>% 
            ...
            tidyr::complete(source_target, interaction) %>% 
            dplyr::select(-dplyr::all_of(c("source", "target"))) %>% 
            tidyr::separate(col = "source_target", sep = " \\| ", remove = FALSE, into = c("source", "target")) %>% 
            dplyr::select(-dplyr::all_of(c("ligand.complex", "receptor.complex"))) %>% 
            tidyr::separate(col = "interaction", sep = " \\| ", remove = FALSE, into = c("ligand.complex", "receptor.complex")) %>% 
            dplyr::mutate("aggregate_rank_inv" = ifelse(is.na(.data$aggregate_rank_inv), 0, .data$aggregate_rank_inv))

liana_AS <- interact_df %>%
            dplyr::filter(.data$target %in% c("Astrolike_Astrocytoma", "OPClike_Astrocytoma"))  %>% 
            dplyr::mutate("target" = ifelse(.data$target == "Astrolike_Astrocytoma", "Astro-like", "OPC-like")) %>% 
            tidyr::unite(c("ligand.complex", "receptor.complex"), col = "interaction", sep = " | ", remove = FALSE) %>%
            tidyr:::unite(c("source", "target"), col = "source_target", remove = FALSE, sep = " | ")  %>% 
            tidyr::complete(source_target, interaction) %>% 
            dplyr::select(-dplyr::all_of(c("source", "target"))) %>% 
...
saveRDS(liana_OD, "/omics/odcf/analysis/OE0145_projects/idh_gliomas/Figures_Science/revision/IDH_gliomas_book/datasets/interactions_OD_microglia_tumor.rds", compress = FALSE)
saveRDS(liana_AS, "/omics/odcf/analysis/OE0145_projects/idh_gliomas/Figures_Science/revision/IDH_gliomas_book/datasets/interactions_AS_microglia_tumor.rds", compress = FALSE)
```
...
#| message: false
SCpubr::save_Plot(p,
                  figure_path = "/omics/odcf/analysis/OE0145_projects/idh_gliomas/Figures_Science/revision/IDH_gliomas_book/figs/Figure_3G",
                  file_name = "Figure_3G",
                  dpi = 300,
...
<img src="../figs/Figure_3G/Figure_3G.png" class="mx-auto d-block" style="box-shadow: none; width: 100%"/>
