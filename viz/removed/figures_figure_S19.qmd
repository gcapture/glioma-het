...
sample_id <- "Microglia paired RNA"
```
...
```{r}
sample@meta.data %>% 
dplyr::select(dplyr::all_of(c("orig.ident", "time", "grade", "pair", "nFeature_RNA", "nCount_RNA"))) %>% 
tibble::as_tibble() %>% 
...
gt::gt()  %>% 
gt::cols_align(columns = c("orig.ident", "grade", "time", "pair"),
               align = "center") %>% 
