...
sample_id <- "Microglia primary RNA"
```
...
```{r}
sample@meta.data %>% 
dplyr::select(dplyr::all_of(c("orig.ident", "subtype", "grade", "nFeature_RNA", "nCount_RNA"))) %>% 
tibble::as_tibble() %>% 
dplyr::group_by(.data$orig.ident) %>% 
dplyr::reframe("subtype" = unique(.data$subtype),
...
gt::gt()  %>% 
gt::cols_align(columns = c("orig.ident", "grade", "subtype"),
               align = "center") %>% 
...
data <- sample@meta.data %>% 
        dplyr::select(dplyr::all_of(c("orig.ident", "final_annotation"))) %>% 
        tibble::as_tibble() %>% 
        dplyr::group_by(.data$orig.ident, .data$final_annotation) %>% 
        dplyr::reframe("cells" = dplyr::n()) %>% 
        tidyr::pivot_wider(names_from = "final_annotation",
                           values_from = "cells") %>% 
        as.data.frame()
...
gt::cols_align(columns = c("orig.ident"),
               align = "center") %>% 
gt::cols_align(columns = unique(sample$final_annotation),
               align = "center") %>% 
gt::tab_style(locations = gt::cells_column_labels(),
              style = list(gt::cell_text(weight = "bold",
                                         v_align = "middle",
                                         align = "center"))) %>% 
gt::tab_style(locations = gt::cells_body(columns = unique(sample$final_annotation)),
              style = list(gt::cell_text(style = "italic",
                                         color = "#00798c"))) %>% 
gt::tab_style(locations = gt::cells_title(),
              style = list(gt::cell_text(weight = "bold",
                                         v_align = "middle"))) %>% 
gt::fmt_number(columns = unique(sample$final_annotation), 
               decimals = 0,
               sep_mark = " ",
               dec_mark = ".") %>% 
gt::cols_label(orig.ident = "Patient") %>% 
